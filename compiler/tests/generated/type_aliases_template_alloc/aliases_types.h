...
namespace tests
{
    template <typename N, typename T, typename _Alloc>
    using array = std::vector<T, typename std::allocator_traits<_Alloc>::template rebind_alloc<T> >;
    ...
    template <typename T, typename _Alloc=arena>
    struct Foo
    {
    ...
    struct WrappingAnEnum
    {
        ...
        explicit
        WrappingAnEnum(const _Alloc& allocator)
          : aWrappedEnum(::tests::_bond_enumerators::EnumToWrap::anEnumValue)
        {
        }
...
namespace std
{
    template<typename _AllocTo, typename T, typename _Alloc>
